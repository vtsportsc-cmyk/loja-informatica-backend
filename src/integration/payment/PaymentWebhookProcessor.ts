...
import type { SuriApi } from '../suri/SuriApi.js';
...
// e notifica o vendedor via SURI. O vendedor fica responsavel pela separacao
// fisica e emissao da NF.
export class PaymentWebhookProcessor {
  private readonly paymentClient: PaymentClient;
  private readonly erpClient: ErpClient;
  private readonly suriApi: SuriApi;
  ...
  async process(rawBody: string, signatureHeader?: string): Promise<PaymentWebhookResult> {
    ...
    await this.suriApi.updateTicket({
      ticketId: session.ticketId,
      status: 'payment_confirmed',
      note: 'Pagamento confirmado. Pedido em "Aguardando NF" - separacao e emissao da NF feitas pelo vendedor.',
      meta: { orderId: order.orderId, botState: fsm.current },
    });

    await this.suriApi.sendReply({
      ticketId: session.ticketId,
      channel: 'whatsapp',
      type: 'text',
  ...
  private async paymentLifecycle(
    ...
    const note =
      kind === 'expired'
        ? 'Cobranca PIX expirou. Estoque liberado; carrinho pode ser retomado.'
        : 'Pagamento cancelado pelo cliente. Estoque liberado.';
    await this.suriApi.updateTicket({
      ticketId,
      status: 'open',
      note,
      meta: { botState: fsm.current, releasedLocks },
    });
    await this.suriApi.sendReply({
      ticketId,
      channel: 'whatsapp',
