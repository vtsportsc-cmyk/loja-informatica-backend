...
import { mockSuriInbound, ScriptedRouter } from './mocks/events.js';
...
describe('MessageHandler - recalculo de carrinho mantem o CEP', () => {
  ...
  it('segundo calculate_cart sem CEP reutiliza o CEP da sessao (fix)', async () => {
    ...
    const inbound = (text: string): SuriInbound =>
      mockSuriInbound({
    ...
    const first = await sessionStore.get('TKT-1001');
    ...
    const second = await sessionStore.get('TKT-1001');
