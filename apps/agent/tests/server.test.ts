...
import { EvolutionWebhookHandler } from '../src/webhooks/evolution.js';
import { PanelApi } from '../src/panel/PanelApi.js';
import { InMemoryMessageRepository } from '../src/prisma/MessageRepository.js';
import { BlingClient } from '../src/integration/bling/BlingClient.js';
import { BlingOrderService } from '../src/integration/bling/BlingOrderService.js';
...
async function startApp(script: ReturnType<typeof fullPurchaseScript> = fullPurchaseScript()): Promise<TestApp> {
  ...
  const panelApi = new PanelApi({
    repository,
    evolution,
    bling: new BlingOrderService({ bling: new BlingClient({ accessToken: '' }) }),
    apiKey: AGENT_KEY,
  });
...
describe('Server HTTP - health, webhooks e API do painel (e2e)', () => {
  let app: TestApp;
  ...
  it('POST /webhooks/whatsapp processa a mensagem, persiste a sessao e responde ao cliente', async () => {
    const res = await fetch(`${app.base}/webhooks/whatsapp`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: mockEvolutionMessage('Oi, quero montar um PC gamer'),
    });
    expect(res.status).toBe(200);
    ...
    const session = await app.sessionStore.get(PHONE);
    ...
    expect(app.evolutionCalls.some((c) => c.includes('/message/sendText/loja'))).toBe(true);
  });

  it('POST /webhooks/whatsapp rejeita corpo invalido com 400', async () => {
    const res = await fetch(`${app.base}/webhooks/whatsapp`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  it('POST /webhooks/payment confirma o PIX, cria pedido e notifica o cliente via Evolution', async () => {
    await seedPending(app.sessionStore, app.erp);
    const rawBody = JSON.stringify(
      pixApprovedNotification({ ticketId: PHONE, amountCents: 272480 }),
    );
  ...
  it('API do painel: POST /api/messages exige x-agent-key (401)', async () => {
    const res = await fetch(`${app.base}/api/messages`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ conversationId: 'conv-1', text: 'Ola!' }),
    });
    expect(res.status).toBe(401);
  });

  it('API do painel: vendedor envia mensagem ao cliente', async () => {
    const conv = await app.container.repository.getConversationByWhatsapp(PHONE);
    expect(conv).not.toBeNull();

    const res = await fetch(`${app.base}/api/messages`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-agent-key': AGENT_KEY },
      body: JSON.stringify({ conversationId: conv!.id, agentId: 'agente-1', text: 'Ola! Seguimos com o pedido?' }),
    });
    expect(res.status).toBe(200);
    expect(app.evolutionCalls.some((c) => c.includes('/message/sendText/loja'))).toBe(true);
  });

  it('API do painel: handoff assume e libera o atendimento', async () => {
    const conv = await app.container.repository.getConversationByWhatsapp(PHONE);
    expect(conv).not.toBeNull();

    const assume = await fetch(`${app.base}/api/conversations/${conv!.id}/handoff`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-agent-key': AGENT_KEY },
      body: JSON.stringify({ action: 'assume', agentId: 'agente-1' }),
    });
    expect(assume.status).toBe(200);
    expect((await app.container.repository.getConversationById(conv!.id))?.humanMode).toBe(true);

    const release = await fetch(`${app.base}/api/conversations/${conv!.id}/handoff`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-agent-key': AGENT_KEY },
      body: JSON.stringify({ action: 'release' }),
    });
    expect(release.status).toBe(200);
    expect((await app.container.repository.getConversationById(conv!.id))?.humanMode).toBe(false);
  });

  it('API do painel: setStatus valida o funil e rejeita status invalido', async () => {
    const conv = await app.container.repository.getConversationByWhatsapp(PHONE);
    expect(conv).not.toBeNull();

    const okRes = await fetch(`${app.base}/api/conversations/${conv!.id}/status`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-agent-key': AGENT_KEY },
      body: JSON.stringify({ status: 'AGUARDANDO_NF' }),
    });
    expect(okRes.status).toBe(200);
    expect((await app.container.repository.getConversationById(conv!.id))?.funnelStatus).toBe('AGUARDANDO_NF');

    const badRes = await fetch(`${app.base}/api/conversations/${conv!.id}/status`, {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-agent-key': AGENT_KEY },
      body: JSON.stringify({ status: 'NAO_EXISTE' }),
    });
