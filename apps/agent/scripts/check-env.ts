...
function main(): void {
  ...
  try {
    ...
    console.log('\n@loja/agent - pre-checagem de ambiente');
...
function buildRows(env: EnvConfig): Row[] {
  ...
  return [
    ...
    { key: 'EVOLUTION_BASE_URL', status: 'ok', value: env.evolution.baseURL },
    { key: 'EVOLUTION_INSTANCE', status: 'ok', value: env.evolution.instance },
    optional('EVOLUTION_API_KEY', Boolean(env.evolution.apiKey), 'configurado'),
    optional('EVOLUTION_WEBHOOK_SECRET', Boolean(env.evolution.webhookSecret), 'configurado (webhook assinado)'),
    { key: 'DATABASE_URL', status: 'ok', value: env.databaseUrl },
    { key: 'AGENT_API_KEY', status: 'ok', value: env.agentApiKey },
    { key: 'STORE_WHATSAPP_NUMBER', status: 'ok', value: env.storeWhatsappNumber },
    { key: 'BLING_BASE_URL', status: 'ok', value: env.bling.baseURL },
    optional('BLING_ACCESS_TOKEN', Boolean(env.bling.accessToken), 'configurado'),
